#include "featureKeeperSmall.h"


void featureKeeperSmall::neuronCreatedFromPattern(unsigned int patternIndex)
{
	_structure->at(patternIndex)->added = true ;
}

bool featureKeeperSmall::hasNeuronCreatedFromPatter(unsigned int patternIndex)
{
 ...
 */
bool featureKeeperSmall::isUniquePattern_information(featureSmall* pat , float thres , short bitsetUsed) // thres should be between 0 and 1 
{
 ...
 */
float featureKeeperSmall::similarityInformationContent(featureSmall * pat, short bitsetUsed) // bitsetUsed specifies the number of bits used .. Used to calculate the average. 
{
...
featureKeeperSmall::featureKeeperSmall() : error(false)
{
	if(__debug__)
	{
	...
	_structure = new std::vector<featureSmall*>(0);
}

featureKeeperSmall::~featureKeeperSmall()
{
 ...
 */
int featureKeeperSmall::isUniquePattern(featureSmall *pat)
{
	for( unsigned int  i= 0 ; i < _structure->size() ; ++i)
	{
		if(_structure->at(i)->pattern == pat->pattern) // We have seen this pattern before , so not unique and do not reach any more. 
		{
			...
			return i ; // We do not add this pattern to the featureKeeperSmall. 
		}
	}
...
bool featureKeeperSmall::insertPattern(featureSmall *newPat)
{
	
	_structure->push_back(newPat);
	if(__debug__)
	{
	//	debugN("featureKeeperSmall"); debug("New Pattern Inseted");
...
int featureKeeperSmall::getFrequency(unsigned int patternIndex)
{
