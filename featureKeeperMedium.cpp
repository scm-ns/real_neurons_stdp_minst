#include "featureKeeperMedium.h"


void featureKeeperMedium::neuronCreatedFromPattern(unsigned int patternIndex)
{
	_structure->at(patternIndex)->added = true ;
}

bool featureKeeperMedium::hasNeuronCreatedFromPatter(unsigned int patternIndex)
{
 ...
 */
bool featureKeeperMedium::isUniquePattern_information(featureMedium* pat , float thres , short bitsetUsed) // thres should be between 0 and 1 
{
 ...
 */
float featureKeeperMedium::similarityInformationContent(featureMedium * pat, short bitsetUsed) // bitsetUsed specifies the number of bits used .. Used to calculate the average. 
{
...
featureKeeperMedium::featureKeeperMedium() : error(false)
{
	if(__debug__)
	{
		std::cout << "FEATURE KEEPER CREATED " << std::endl;
	}

	_structure = new std::vector<featureMedium*>(0);
}

featureKeeperMedium::~featureKeeperMedium()
{
 ...
 */
int featureKeeperMedium::isUniquePattern(featureMedium *pat)
{
	for( unsigned int  i= 0 ; i < _structure->size() ; ++i)
	{
		if(_structure->at(i)->pattern == pat->pattern) // We have seen this pattern before , so not unique and do not reach any more. 
		{
			...
			return i ; // We do not add this pattern to the featureKeeperMedium. 
		}
	}
...
bool featureKeeperMedium::insertPattern(featureMedium *newPat)
{
	
	_structure->push_back(newPat);
	if(__debug__)
	{
	//	debugN("featureKeeperMedium"); debug("New Pattern Inseted");
...
int featureKeeperMedium::getFrequency(unsigned int patternIndex)
{
